...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if (isPostCreate) return NextResponse.json({error: `Post is created`}, {status: 404})
        ...
        return NextResponse.json({success: true, url: newPost.url}, {status: 201});
    } catch (err: any) {
        console.error(err);
        const firstErrorKey = Object.keys(err.errors)[0];
        const firstErrorObject = err.errors[firstErrorKey];
        return NextResponse.json({error: `Something went wrong! ${firstErrorObject.path} ${firstErrorObject.kind}`}, {status: 500});
    }
}